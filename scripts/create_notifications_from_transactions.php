...
$sql = "SELECT transaction_id FROM pding_transactions WHERE notification_queued = 0";
...
function createNotification($ids, $msgType) {
	...
	//Execute query
	$sql = "SELECT * from pding_transactions WHERE transaction_id IN($transaction_ids) ORDER BY subtotal ASC";
	...
	$msgs = array(
		0 => array(
			...
			"action" => $path . "/view_order.php?ids=" . $transaction_ids,
	...
	foreach($ids AS $id) {
		$sql = "UPDATE pding_transactions SET notification_queued=1 WHERE transaction_id = $id";
		mysqli_query($conn, $sql);
	}
